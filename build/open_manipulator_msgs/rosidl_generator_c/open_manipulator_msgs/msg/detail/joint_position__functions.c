  ...
  open_manipulator_msgs__msg__JointPosition__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    open_manipulator_msgs__msg__JointPosition * data =
      (open_manipulator_msgs__msg__JointPosition *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!open_manipulator_msgs__msg__JointPosition__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          open_manipulator_msgs__msg__JointPosition__fini(&data[i]);
        }
        free(data);
        return false;
      }
