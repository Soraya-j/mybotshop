...
import logging
import threading
import go2_zt30.siyi_sdk.cameras
from go2_zt30.siyi_sdk.utils import  toInt
from time import sleep, time
from go2_zt30.siyi_sdk.siyi_message import *
...
class SIYISDK:
    ...
    def bufferCallback(self):
        ...
        while(len(buff_str)>=MINIMUM_DATA_LENGTH):
            ...
            elif cmd_id==COMMAND.IMAGE_MODE_SET:
                self.parseImageModeMsg(data, seq)
            else:
                self._logger.warning(f"CMD ID is not recognized: {cmd_id}")
    ...
    def setImageMode(self, mode: int):
        msg = self._out_msg.setImageMode(mode)
        self.sendMsg(msg)
    ...
    #                Parsing functions                 #
    ####################################################
    def parseImageModeMsg(self, msg: str, seq: int):
        """
        Parse the image mode response message (CMD 0x11).
        msg: hex string
        seq: sequence number
        """
        try:
            mode_val = int(msg[0:2], 16)
            self._logger.info(f"Image mode set: {self.decodeImageMode(mode_val)}")
            return True
        except Exception as e:
            self._logger.error(f"Failed to parse image mode message: {e}")
            return False
    ...
    def decodeImageMode(self, mode):
        mode_dict = {
            0x00: "Wide Main, Zoom Sub",
            0x01: "Zoom Main, Wide Sub",
            0x02: "Wide Only",
            0x03: "Zoom Only",
            0x04: "PIP Wide Main, Zoom Sub",
            0x05: "PIP Zoom Main, Wide Sub",
            0x06: "Dual Split (Wide + Zoom)",
            0x07: "Thermal Main, Zoom Sub",
            0x08: "Thermal Main, Wide Sub",
            0x09: "Thermal Only",
            0x0A: "Dual Split (Thermal + Visible)",
            0x0B: "PIP Thermal Main, Zoom Sub"
        }
        return mode_dict.get(mode, f"Unknown mode ({mode})")

    #################################################
    #                 Set functions                 #
    #################################################
    def setGimbalRotation(self, yaw, pitch, err_thresh=1.0, kp=3.0, kd=2.0):
        """
        Sets gimbal attitude angles using PD control (Proportional + Derivative)
        ...
        previous_yaw_err = 0.0
        previous_pitch_err = 0.0

        while True:
            self.requestGimbalAttitude()
            if self._att_msg.seq == self._last_att_seq:
                self._logger.info("No new attitude message received")
            ...
            current_yaw = self._att_msg.yaw
            current_pitch = self._att_msg.pitch

            yaw_err = -yaw + current_yaw  # NOTE: reversed
            pitch_err = pitch - current_pitch

            dyaw = yaw_err - previous_yaw_err
            dpitch = pitch_err - previous_pitch_err

            y_speed_sp = int(kp * yaw_err + kd * dyaw)
            p_speed_sp = int(kp * pitch_err + kd * dpitch)

            y_speed_sp = max(min(100, y_speed_sp), -100)
            p_speed_sp = max(min(100, p_speed_sp), -100)

            self._logger.debug(f"Yaw error: {yaw_err}, Pitch error: {pitch_err}")
            self._logger.debug(f"Yaw speed: {y_speed_sp}, Pitch speed: {p_speed_sp}")

            if abs(yaw_err) <= err_thresh and abs(pitch_err) <= err_thresh:
                self.requestGimbalSpeed(0, 0)
                self._logger.info("Target gimbal rotation reached.")
                break

            self.requestGimbalSpeed(y_speed_sp, p_speed_sp)

            previous_yaw_err = yaw_err
            previous_pitch_err = pitch_err

            sleep(0.2)  # control loop delay
...
def test():
    ...
    print("Current zoom level: ", cam.getCurrentZoomLevel())
