...
import logging
from os import stat
from go2_zt30.siyi_sdk.utils import toHex
from go2_zt30.siyi_sdk.crc16_python import crc16_str_swap
...
class SIYIMESSAGE:
    ...
    def setImageMode(self, mode_index: int):
        """
        Set image display mode. ZT30 supports 0~8 combinations of main/sub camera views.
        """
        if mode_index < 0 or mode_index > 13:
            self._logger.error("Invalid image mode index.")
            return ''
        data = toHex(mode_index, 8)
        cmd_id = COMMAND.IMAGE_MODE_SET
        return self.encodeMsg(data, cmd_id)
