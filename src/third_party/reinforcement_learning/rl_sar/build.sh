...
run_cmake_build() {
    ...
    # Get Jetson-specific CMake arguments if on Jetson platform
    local jetson_args=""
    if [ "$IS_JETSON" = true ]; then
        jetson_args=$(get_jetson_cmake_python_args)
        if [ -n "$jetson_args" ]; then
            print_info "Using Jetson Python configuration"
        fi
    fi

    cmake src/rl_sar/ -B cmake_build -DUSE_CMAKE=ON $jetson_args
...
run_mujoco_build() {
    ...
    cmake src/rl_sar/ -B cmake_build -DUSE_CMAKE=ON -DUSE_MUJOCO=ON $jetson_args
    cmake --build cmake_build -j$(nproc 2>/dev/null || echo 4)
...
show_usage() {
    ...
    echo -e "  -r, --ros        Build using ROS (for simulation only)"
    echo -e "  -mj,--mujoco     Build with MuJoCo simulator support"
    echo -e "  -h, --help       Show this help message"
    echo ""
    echo -e "${COLOR_INFO}Examples:${COLOR_RESET}"
    echo -e "  $0                    # Build for real robot (default)"
    echo -e "  $0 -mj                # Build with MuJoCo simulator support"
    echo -e "  $0 -r                 # Build ROS packages for Gazebo simulation"
    echo -e "  $0 -r package1        # Build specific ROS packages"
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -r|--ros) ros_mode=true; shift ;;
            -m|--cmake) shift ;;  # Deprecated, CMake is now default
    ...
    # Handle ROS build mode
    if [ "$ros_mode" = true ]; then
    ...
    # Default: Build for real robot (CMake mode)
    # Warn if ROS environment is sourced (can cause DDS conflicts)
    if [ -n "$ROS_DISTRO" ]; then
        print_warning "ROS environment detected (ROS_DISTRO=$ROS_DISTRO)"
        print_warning "Building for real robot may have DDS conflicts with ROS."
        print_info "Consider running in a clean terminal without ROS sourced."
        print_info "Unsetting ROS-related environment variables..."
        unset ROS_DISTRO
    ...
    # Default CMake build for real robot
    setup_inference_runtime
    run_cmake_build
    exit 0
}
