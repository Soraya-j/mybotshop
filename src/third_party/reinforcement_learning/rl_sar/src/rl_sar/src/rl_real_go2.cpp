...
void RL_Real::GetState(RobotState<float> *state)
{
    ...
    // Only use joystick values if joystick is actively being used (deadzone threshold)
    // This allows keyboard control to work when joystick is idle
    constexpr float JOYSTICK_DEADZONE = 0.05f;
    float joy_x = this->joystick.ly();
    float joy_y = -this->joystick.lx();
    float joy_yaw = -this->joystick.rx();

    if (std::abs(joy_x) > JOYSTICK_DEADZONE ||
        std::abs(joy_y) > JOYSTICK_DEADZONE ||
        std::abs(joy_yaw) > JOYSTICK_DEADZONE)
    {
        // Joystick is active, use joystick values
        this->control.x = joy_x;
        this->control.y = joy_y;
        this->control.yaw = joy_yaw;
    }
    // Otherwise, keep existing control.x/y/yaw values (set by keyboard)
